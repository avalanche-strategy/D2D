...
Our evaluation is inspired by the RAGAS framework, which provides five main metrics - faithfulness, relevance, precision, recall, and correctness (when golden answers are available) to evaluate the model performance compenhensively[1]. However, RAGAS has some limitations when applied to Dialogue2Data, which we aim to address using strategies proposed in Daedalus v4[2].
